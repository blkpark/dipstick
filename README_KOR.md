...
dipstick 은 이 작업에 어떤 모델을 쓸지, 추론을 얼마나 깊게 굴릴지 정해주지 않는다. 그건
*작업이 뭔지* 를 정하는 쪽 — 채점표든 스킬이든 사람 판단이든 — 의 몫이고, 계기를 보기
전에 이미 정해져 있어야 한다. 계기는 그 결정에 대해 딱 하나만 답한다. **지금 돌릴 수 있나?**

둘을 갈라놓는 이유가 있다. 잔량이 부족할 때 이 둘이 같은 얘기처럼 보이지만 아니다. "이
작업엔 깊은 모델이 필요하다" 는 작업에 대한 말이고, "그 풀은 4% 남았다" 는 달력에 대한
말이다. 뒤가 앞을 고쳐 쓰게 두면, 큰 모델이 필요했던 작업이 조용히 작은 모델로 나가고
로그엔 아무 흔적도 안 남는다. 그래서 `can_i_start` 는 GO, 아니면 회복 시각을 붙인 WAIT
만 답한다. "대신 작은 모델 쓰세요" 는 없다.

계기가 모델을 아는 지점은 잔량 쪽이다. 요금제에는 공용 창 말고 모델 전용 상한이 따로
붙기도 한다 — Claude Max 는 Fable 을 따로 재고, agy 는 모델 계열별로 풀이 나뉜다. 그래서
계정 전체는 멀쩡한데 고른 모델만 막혀 있을 수 있고, 반대도 된다.
`can_i_start(minutes, model: "...")` 는 그 모델 전용 창에 같이 깎이는 공용 창까지만 보고
판정하고, 패널에서도 그 상한이 각자 한 줄씩 선다.

순서로 두면 이렇다.

1. 작업을 보고 모델과 effort 를 정한다 — 범위, 파급, 새로움.
2. 그 선택이 지금 돌아가는지 계기에 묻는다.
3. GO 면 발사. WAIT 면 기다리거나 다른 작업을 한다. **맞추려고 내리지 않는다.**

effort 는 아예 dipstick 까지 오지 않는다. 실행이 얼마나 태우는지를 바꿀 뿐이라, 나중에
토큰 이력에 숫자로만 남는다. 설정값 자체는 계기가 볼 수 없다.
