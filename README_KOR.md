...
다음 작업은 어느 계정으로 쏘나? dipstick 은 이 기계의 AI 코딩 구독을 전부 읽고 그 질문에
답한다 — 메뉴바에서, 명령줄에서, MCP 로 에이전트에게도.

Claude Code, Codex, Antigravity 는 각자 다른 창(window)으로 사용량을 재고, 각자 다른 데
표시하고, 긴 작업을 걸기 전에 정작 중요한 두 질문 — *어디에 여유가 있나, 없다면 언제
돌아오나* — 에는 아무도 답하지 않는다. 계정을 둘 이상 쓰면 둘째 질문이 첫째 질문이 된다.
발사마다 탈 풀을 누군가 골라야 하고, 감으로 고르면 한 구독은 말라붙는 동안 다른 하나는
놀고 있다. dipstick 은 세 벤더를 한 자리에서 읽고 답을 상시 유지한다. 아무 명령 앞에
`$(dipstick --main-cmd)` 를 붙이면, 숫자가 태우라는 계정으로 발사된다.
