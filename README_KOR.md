...
AI 코딩 구독을 여러 개 쓰면 매번 같은 고민이 생긴다. **"이번 작업, 어느 계정으로 돌리지?"**
dipstick 이 그걸 대신 정해준다.

Claude Code, Codex, Antigravity 가 각각 얼마나 남았는지 메뉴바 한 자리에 모아 보여주고,
남은 양과 리셋 시각을 따져서 "지금은 이 계정" 이라고 답한다. 명령 앞에
`$(dipstick --main-cmd)` 만 붙이면 그 계정으로 실행된다.

고르는 방식은 두 가지다.

- **weighted (자동)** — 여유가 가장 많은 계정을 골라준다. 단순히 퍼센트가 높은 쪽이 아니라
  리셋까지 남은 시간을 같이 본다. 내일 리셋되는 40% 가, 방금 리셋돼서 앞으로 일주일을
  버텨야 하는 94% 보다 여유 있는 쪽이다. 이걸 *페이스* 라고 부른다.
- **pinned (고정)** — 계산 없이, 내가 정한 계정 하나만 계속 태운다.

**혼자서 돌아간다.** Orca 같은 오케스트레이터도, 데몬도, 별도 패키지도 필요 없다. 이미
Orca 를 쓰고 있다면 그때는 같이 물린다 ([Orca 같은 다계정 래퍼와 함께 쓸 때](#orca-같은-다계정-래퍼와-함께-쓸-때)).
